...
export default function ComparadorPage() {
  ...
  // ── Generar items dinámicos desde los datos ──────────────────────────────────
  const grupos = useMemo(() => {
    ...
    // Deudas: cada deuda
    const deuItems = (deudas??[]).map((d, idx) => ({
      id: `d_${d.id}`, label: d.nombre,
      color: DEU_COLORS[idx % DEU_COLORS.length], grupoId: 'deudas',
    }))

    // Tarjetas: cada tarjeta
    const tarItems = (tarjetas??[]).map((t, idx) => ({
      id: `t_${t.id}`, label: t.nombre,
  ...
  // ── Calcular datos por métrica ───────────────────────────────────────────────
  const metricData = useMemo(() => {
    const map: Record<string, number[]> = {}

    // Ingresos por tipo
    grupos.find(g => g.id === 'ingresos')?.items.forEach(item => {
      const tipo = item.id.replace('i_', '')
      map[item.id] = MESES_CORTOS.map((_, i) =>
        (ingresos??[]).filter(x => x.mes === i+1 && x.tipo === tipo).reduce((s, x) => s + x.monto, 0)
      )
    })

    // Egresos por categoría
    grupos.find(g => g.id === 'egresos')?.items.forEach(item => {
      const cat = item.id.replace('e_', '')
      map[item.id] = MESES_CORTOS.map((_, i) =>
        (egresos??[]).filter(x => x.mes === i+1 && x.categoria === cat).reduce((s, x) => s + x.monto, 0)
      )
    })

    // Deudas por cuota mensual
    grupos.find(g => g.id === 'deudas')?.items.forEach(item => {
      const deudaId = item.id.replace('d_', '')
      const d = (deudas??[]).find(x => x.id === deudaId)
      map[item.id] = MESES_CORTOS.map(() => d?.cuota_mensual ?? 0)
    })

    // Tarjetas por pagos
    grupos.find(g => g.id === 'tarjetas')?.items.forEach(item => {
      const tarjetaId = item.id.replace('t_', '')
      map[item.id] = MESES_CORTOS.map((_, i) =>
        (pagos??[]).find(p => p.tarjeta_id === tarjetaId && p.mes === i+1)?.monto ?? 0
      )
    })
  ...
  const toggleItem  = (id: string) => setActive(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n })
  const toggleMes   = (i: number)  => setActiveMeses(prev => { if (prev.size===1&&prev.has(i)) return prev; const n = new Set(prev); n.has(i)?n.delete(i):n.add(i); return n })
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 flex-wrap mb-4">
        ...
        {active.size > 0 && (
          <button onClick={clearAll} className="text-xs text-slate-400 hover:text-slate-600 underline border-none bg-transparent cursor-pointer">
            Limpiar todo
          </button>
        )}
          ...
          <ResponsiveContainer width="100%" height={280}>
            {chartType === 'bar' ? (
              <BarChart data={chartData} barCategoryGap="25%" barGap={2}>
                ...
                <Tooltip contentStyle={TT} formatter={(v:number,n:string) => [fmt(v,m), allItems.find(i=>i.id===n)?.label||n]} />
                {activeItems.map(i => (
                  <Bar key={i.id} dataKey={i.id} name={i.id} fill={i.color+'dd'} radius={0} maxBarSize={40} stackId={getGrupoId(i.id)} />
                ))}
              </BarChart>
            ) : (
