      ...
      {top5.map(p => {
        ...
        return (
          <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 11, fontFamily: 'monospace', fontWeight: 700, color: '#0f172a' }}>{fmt(p.value as number, m)} <span style={{ fontWeight: 400, color: '#94a3b8' }}>({total>0?Math.round((p.value as number)/total*100):0}%)</span></span>
          </div>
        )
      })}
      {top5.length > 1 && (
  ...
  const filaHTML = (r: DraftRow, onChange: (patch: Partial<DraftRow>) => void, onEnter: () => void, startRow: number, key: string, onEscape: () => void, onDescartar?: () => void, flash?: boolean) => {
    ...
    <tr key={key} className={`transition-colors duration-500 ${flash ? 'bg-emerald-100' : filaConAlgo(r) ? 'bg-amber-50/50' : ''}`}>
      ...
      <td className="border border-slate-200" style={{width:150}}>
        <CategoriaSelector bare modulo="ingresos" value={r.tipo} onChange={v => onChange({ tipo: v })} frecuencia={frecuencia} onPaste={handlePasteEnCelda(startRow, 2)}
          onKeyDown={handleKey}
...
// ─── InlineEditRow ────────────────────────────────────────────────────────────
function InlineEditRow({ ingreso, tiposBase, categoriasCustom, frecuencia, descripciones, quienOpts, onSave, onCancel, refetchCats }: {
  ingreso: Ingreso
  tiposBase: { key: string; label: string; icon: string; color: string }[]
  categoriasCustom: CategoriaCustom[]
  frecuencia?: Record<string, number>
...
export default function IngresosPage() {
  ...
  const getWidgetValue = (id: string) => {
    switch (id) {
      ...
      case 'salarios':      return { value: fmt(salarios, m), sub: `${total > 0 ? Math.round(salarios / total * 100) : 0}% del total`, trend: trendSalarios, trendLabel: trendSalariosLabel, color: '#40B046' }
      case 'extra':         return { value: fmt(total - salarios, m), sub: 'Freelance + alquiler + otros', trend: trendExtra, trendLabel: trendExtraLabel, color: '#52A852' }
      case 'promedio':      return { value: fmt(promedio, m), sub: 'Sobre meses con datos', trend: trendPromedio, trendLabel: 'vs promedio año anterior', color: '#1A5E9E' }
      case 'top_categoria': return { value: topAño[0]?.label ?? '—', sub: topAño[0] ? fmt(topAño[0].value, m) : 'Sin datos', trend: trendTop.trend, trendLabel: trendTop.label, color: topAño[0]?.color ?? '#888780' }
      case 'cantidad':      return { value: String(data.length), sub: 'Ingresos registrados', trend: trendCantidad, trendLabel: trendCantidadLabel, color: '#5B3FA6' }
      ...
      <Modal open={showModal} onClose={() => { setShowModal(false); setForm(FORM_INIT); setModalEditId(null) }} title={modalEditId ? 'Editar ingreso' : 'Nuevo ingreso'}>
        <div className="flex flex-col gap-4">
          <div><FieldLabel>Tipo</FieldLabel>
            <CategoriaSelector modulo="ingresos" value={form.tipo} onChange={v => setForm(p => ({ ...p, tipo: v }))} frecuencia={frecuenciaCats.data ?? undefined}
              categorias={categoriasCustom} categoriasBase={tiposBase} onCategoriasChange={refetchCats} />
          </div>
