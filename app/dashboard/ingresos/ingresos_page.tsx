...
import type { Moneda, Quien, Ingreso } from '@/types'
...
function InlineEditRow({ ingreso, tiposBase, categoriasCustom, onSave, onCancel, refetchCats }: {
  ...
  categoriasCustom: { id:string; nombre:string; icono:string; color:string; parent_id:string|null; modulo:string; user_id:string; created_at:string; children?:unknown[] }[] | null
...
export default function IngresosPage() {
  ...
  const { data: categoriasCustom, refetch: refetchCats } = useCategoriasCustom('ingresos')
  ...
  const tiposBase = useMemo(() => Object.entries(TIPOS_INGRESO).map(([key,cfg])=>({key,label:cfg.label,icon:cfg.icon,color:cfg.color})), [])

  const allTipos = useMemo(() => {
    const flat: {key:string;label:string;icon:string;color:string}[] = []
    const traverse = (cats: typeof categoriasCustom, prefix='') => {
      cats?.forEach(c => {
        flat.push({key:c.id, label:prefix+c.nombre, icon:c.icono, color:c.color})
        if (c.children?.length) traverse(c.children as typeof categoriasCustom, prefix+'  ')
      })
    }
  ...
  const getTipoInfo = (tipo:string) => allTipos.find(t=>t.key===tipo) ?? {key:tipo,label:tipo,icon:'📦',color:'#888780'}
  ...
  const compData = useMemo(()=>{
    ...
    return allTipos.map(t=>({name:t.label,color:t.color,value:src.filter(i=>i.tipo===t.key).reduce((s,i)=>s+i.monto,0)})).filter(d=>d.value>0)
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {filtered.map((ingreso,rowIdx)=>{
                  ...
                  const bg = isEven ? '' : 'bg-slate-50/70'
