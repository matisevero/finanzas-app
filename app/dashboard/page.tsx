...
export default function DashboardPage() {
  ...
  const [distMes, setDistMes]     = useState(-1) // -1 = acumulado
  ...
  const distData = useMemo(()=>{
    const src = distMes===-1
      ? (distMode==='egresos' ? egresos??[] : ingresos??[])
      : (distMode==='egresos' ? (egresos??[]).filter(x=>x.mes===distMes+1) : (ingresos??[]).filter(x=>x.mes===distMes+1))
    if (distMode==='egresos') {
      return Object.entries(TIPOS_EGRESO).map(([key,cfg])=>({
        name:cfg.label, color:cfg.color,
        value:(src as typeof egresos extends (infer T)[]|null?T:never[]).filter((e:any)=>e.categoria===key).reduce((s:number,e:any)=>s+e.monto,0)
      })).filter(d=>d.value>0).sort((a,b)=>b.value-a.value).slice(0,7)
    } else {
      return Object.entries(TIPOS_INGRESO).map(([key,cfg])=>({
        name:cfg.label, color:cfg.color,
        value:(src as typeof ingresos extends (infer T)[]|null?T:never[]).filter((i:any)=>i.tipo===key).reduce((s:number,i:any)=>s+i.monto,0)
      })).filter(d=>d.value>0)
    }
  ...
  // Tarjetas — último pago por tarjeta
    ...
    <div style={{display:'flex',flexDirection:'column',gap:16,paddingBottom:24}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
        {[
          ...
          {l:'Inversiones USD',     v:fmt(totalUSD,m),      s:'ARS equiv. comprados', t:Math.round(totalUSD/totalEgresos*100||0)+'% del egreso', up:true},
        ].map(k=>(
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
            ...
            <div style={{display:'flex',gap:3,background:'var(--color-background-secondary)',borderRadius:6,padding:3}}>
              {(['bar','line'] as const).map(t=>(
                <button key={t} onClick={()=>setFlowType(t)} style={{border:'none',background:flowType===t?'var(--color-background-primary)':'transparent',fontSize:11,padding:'3px 10px',borderRadius:4,cursor:'pointer',color:flowType===t?'var(--color-text-primary)':'var(--color-text-secondary)',fontWeight:flowType===t?500:'normal',outline:'none'}}>
                  {t==='bar'?'barras':'línea'}
                </button>
              ))}
            </div>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14,gap:12}}>
            <div style={{display:'flex',gap:3,background:'var(--color-background-secondary)',borderRadius:6,padding:3,flexShrink:0}}>
              {(['egresos','ingresos'] as const).map(t=>(
                <button key={t} onClick={()=>setDistMode(t)} style={{border:'none',background:distMode===t?'var(--color-background-primary)':'transparent',fontSize:11,padding:'3px 9px',borderRadius:4,cursor:'pointer',color:distMode===t?'var(--color-text-primary)':'var(--color-text-secondary)',fontWeight:distMode===t?500:'normal',outline:'none'}}>
                  {t}
                </button>
              ))}
            </div>
            <div style={{flex:1,display:'flex',alignItems:'center',gap:8}}>
              <input type="range" min="0" max={MESES_CORTOS.length} step="1" value={distMes===-1?MESES_CORTOS.length:distMes}
