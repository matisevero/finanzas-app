...
export default function SaludConfigModal({
  ...
  // Al mover un slider en modo general, reacomoda los demás proporcionalmente para
  // seguir sumando 100. En modo "este mes" solo cambia el override puntual — no
  // toca los pesos generales de los demás.
  const setPesoGeneral = (id: string, nuevoPeso: number) => {
    setDraft(prev => {
      const cat = prev.find(c => c.id === id)
      if (!cat) return prev
      const otros = prev.filter(c => c.id !== id && !c._eliminar)
      const sumaOtros = otros.reduce((s, c) => s + c.peso, 0)
      const restante = Math.max(0, 100 - nuevoPeso)
      return prev.map(c => {
        if (c.id === id) return { ...c, peso: nuevoPeso }
        if (c._eliminar) return c
        return { ...c, peso: sumaOtros > 0 ? Math.round((c.peso / sumaOtros) * restante * 10) / 10 : restante / Math.max(1, otros.length) }
      })
    })
  }
  ...
  const addCategoria = () => {
    setDraft(prev => [...prev, {
      id: tmpId(), nombre: 'Nueva categoría', icono: '📌', color: '#1A5E9E',
      peso: 0, umbral: 20, comparacion: 'menor_que', fuente_tipo: 'egreso_categoria',
  ...
  return (
    <Modal open={open} onClose={onClose} title="Configurar Salud Financiera">
      {(lc && !categoriasDb) ? <LoadingSpinner /> : (
        <div className="flex flex-col gap-4">
          ...
          <div className={`text-xs rounded-lg px-3 py-2 border ${Math.round(pesoTotal) === 100 ? 'bg-slate-50 border-slate-200 text-slate-500' : 'bg-red-50 border-red-200 text-red-600'}`}>
            ...
            {Math.round(pesoTotal) !== 100 && ' — debería sumar 100'}
          </div>
              ...
              return (
                <div key={c.id} className="border border-slate-200 rounded-xl p-3">
                  ...
                  <div className="flex gap-2 flex-wrap items-center mb-2">
                    <select value={c.fuente_tipo} onChange={e => updateDraft(c.id, { fuente_tipo: e.target.value as SaludFuenteTipo, fuente_config: {} })}
